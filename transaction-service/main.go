...
func processPayment(c *gin.Context) {
	...
	if success {
		...
		err = sendReceiptEmail(form.Email, "Your Payment Receipt", "Please find attached your receipt.", filename)
		if err != nil {
	...
	resultHTML := `<html><body><h1>Payment %s</h1><p>Transaction %s has been %s.</p></body></html>`
...
func sendReceiptEmail(toEmail string, subject string, body string, attachmentPath string) error {
	m := gomail.NewMessage()
	m.SetHeader("From", "ormanov.adik@gmail.com") // Замените на ваш SMTP email
	m.SetHeader("To", toEmail)
	m.SetHeader("Subject", subject)
	m.SetBody("text/plain", body)
	m.Attach(attachmentPath)

	d := gomail.NewDialer("smtp.gmail.com", 587, "ormanov.adik@gmail.com", "sbtrenmxlmcofajm")
...
func showPaymentForm(c *gin.Context) {
	...
	<body>
		<h1>Payment Form for Transaction {{.TransactionID}}</h1>
		<form method="POST" action="/transactions/{{.ID}}/pay">
